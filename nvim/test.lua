local function follow_link()
  ...
  local search_start = cursor_pos[2]
  local open_paren_ix, close_paren_ix = nil, nil

  while true do
    close_paren_ix = string.find(current_line, "%)", search_start)
    ...
    open_paren_ix = FindLast(current_line, "%(", close_paren_ix)
    ...
    local extra_close_paren_ix =
      string.find(string.sub(current_line, open_paren_ix, close_paren_ix - 1), "%)")
    print("extra_close_paren_ix: ", extra_close_paren_ix)
    -- Check open parenthesis preceded by square bracket
    local leading_char = string.sub(current_line, open_paren_ix - 1, open_paren_ix - 1)
  ...
  local address = string.sub(current_line, open_paren_ix + 1, close_paren_ix - 1)
